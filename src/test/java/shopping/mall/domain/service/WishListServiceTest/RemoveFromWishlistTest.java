...
public class RemoveFromWishlistTest {
    ...
    public void sut는_wishList조회시_userId와_요청받은_userId가_다르면_예외가_발생한다(){
        ...
        RemoveWishListItemRequest request = ReflectionProvider.getRemoveWishListItemRequest(requestWishListId, itemId, requestUserId);
        Mockito.when(wishListRepository.findByWishListId(request.getWishListId()))
                .thenReturn(wishList);

        //Act
        try {
            sut.removeFromWishlist(request);
